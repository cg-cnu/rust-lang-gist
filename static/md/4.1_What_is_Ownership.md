...
* str (string literal) -  text is hardcoded directly into the final executable
  ...
  * Size and contents known at compile time, immutable 

* String - Allocated during run time

  * written to heap, slow to allocate and access

  * Size and contents are dynamic, mutable and growable 
...
* Memory allocated in heap at run time during `let mut s = String::from("hello");`
...
* Tedious to tranfer back all the time, use references 
